...
container.set 'acl',container.share (c)->
    ...
    acl.pquery = -> c.q.ninvoke(acl,'query',arguments...)
    return acl
...
container.set 'middlewares',container.share (c)->
    ...
    ###
    inMemorySession:->
        c.express.session(secret:c.secret,name:c.name,cookie:{maxAge:1000*60*60*24,httpOnly:true})
    ###
...
container.set 'EventEmitter',container.share (c)->
    class EventEmitter
        ...
        emit:(eventName,args...)->
            ...
            .reduce(
                ((q, next)-> c.q.then(next.listener.bind(next.listener, args...))),
                @_Q()
            )
...
container.set 'RedisStore',container.share (c)->
    require('connect-redis')(c.express)
